...
> - OCR 서버 호출 및 강의명 검증 단계가 스킵됨
> - 이미지 업로드만으로 수료증 인증 완료
> - 관리자가 수료증을 직접 확인 후 승인/반려
...
6. [조건] 후기가 승인됨 (approvalStatus: APPROVED):
   - 프론트엔드 → 사용자: 수정 버튼 비활성화 또는 알럿 표시 ("승인된 후기는 수정할 수 없습니다")
7. [조건] 후기가 미승인 (approvalStatus: PENDING 또는 REJECTED):
   - 프론트엔드 → 사용자: 후기 수정 폼 표시 (기존 데이터 채워짐)
   - 사용자 → 프론트엔드: 수정된 내용 제출 (별점, 후기 콘텐츠)
   - 프론트엔드 → 백엔드: 후기 수정 API 요청 (PUT /api/reviews/{reviewId})
   - 백엔드: 승인 상태 확인 (APPROVED면 403 반환)
   - 백엔드 → DB: 후기 업데이트
   - DB → 백엔드: 업데이트 완료
   - 백엔드 → 프론트엔드: 200 OK (수정 성공)
   - 프론트엔드 → 사용자: 성공 메시지 표시
```
...
| 후기 작성 | 작성 가능 / 이미 작성됨 |
| 후기 수정 | 승인됨 (APPROVED) → 수정 불가 / 미승인 (PENDING, REJECTED) → 수정 가능 |
...
| 후기 수정 | 로그인 필수 + 작성자 본인만 가능 + **승인되지 않은 후기만 가능** |
| 수료증 인증 | 로그인 필수 |
| 닉네임 설정 | 로그인 필수 |

> **Note**: 사용자는 후기를 삭제할 수 없습니다. 수정만 가능합니다.
> **Note**: 관리자가 승인(APPROVED)한 후기는 수정할 수 없습니다.
