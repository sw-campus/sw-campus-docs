...
SW Campus는 소프트웨어 교육 강의를 비교하고 리뷰할 수 있는 플랫폼입니다. 사용자가 서비스를 이용하기 위해서는 회원 인증 시스템이 필요하며, 일반 사용자/교육 제공자/관리자 각각의 역할에 맞는 접근 제어가 요구됩니다.
...
| 교육 제공자 | `PROVIDER` | 강의 등록 및 관리 (관리자 승인 필요) |
| 관리자 | `ADMIN` | 시스템 전체 관리, 교육 제공자 승인 |
...
| 구분 | 사용자 스토리 |
|------|-------------|
| US-05 | 교육 제공자는 일반 사용자 정보 + 재직증명서 이미지를 첨부하여 가입 신청할 수 있다 |
| US-06 | 교육 제공자는 회원가입 시 `ROLE = PROVIDER`, `ORG_AUTH = 0`(미승인) 상태로 저장된다 |
| US-07 | 교육 제공자는 미승인(`ORG_AUTH = 0`) 상태에서 로그인은 가능하나 강의 등록은 불가하다 |
| US-08 | 교육 제공자는 관리자 승인 후 `ORG_AUTH = 1`(승인) 상태가 되어 강의 등록이 가능하다 |
