...
```
db-postgres/UserEntityRepository
db-mysql/UserEntityRepository (미래에 추가될 수 있음)
...
public class OrderService {
    public void validateEmail(String email) { ... }  // 완전히 동일한 코드
}
...
public class CartService {
    double discount = price * 0.1;  // 같은 로직
}
...
```
깨진 창문 하나 → "어차피 이미 망가졌으니" → 더 많은 창문 깨짐

나쁜 코드 하나 → "어차피 레거시니까" → 코드베이스 전체 부패
```
...
## PR 머지 전 확인사항
- [ ] 코드 규칙 준수 (네이밍, 패키지 구조)
- [ ] 새로운 TODO 있으면 이슈 등록했는가?
- [ ] 기존 깨진 창문을 더 깨뜨리지 않았는가?
- [ ] 불필요한 주석, 사용하지 않는 코드 제거
```

#### 4) 기술 부채 관리

```
발견 → 이슈 등록 → 라벨링 (tech-debt) → 스프린트에 포함
```

---

## 5. 우연에 맡기는 프로그래밍 금지 (Don't Program by Coincidence)
...
✅ 복붙 코드는 반드시 이해 후 사용
✅ "동작함" ≠ "올바름"
✅ 가정을 명시적으로 문서화하거나 테스트로 검증
```

### 코드 리뷰 기준

```
리뷰어: "이 코드가 왜 이렇게 작성되었나요?"
작성자: 명확하게 설명할 수 있어야 함

설명 불가 → 다시 이해하고 오거나, 다른 방법 찾기
```
...
// ❌ 우연에 맡긴 코드
public void process() {
    Thread.sleep(100);  // 이거 빼면 에러나서 넣음
    doSomething();
}

// ✅ 이유를 이해한 코드
public void process() {
    // 외부 API 호출 후 데이터 동기화까지 최소 100ms 필요
    // TODO(2025-01-15): 폴링 방식으로 개선 예정 - #456
    Thread.sleep(100);
    doSomething();
}
```
...
// Domain 객체 생성 테스트
@Test
void 유효한_이메일로_사용자_생성() {
    User user = new User("test@example.com", "password", "홍길동");
    assertThat(user.getEmail()).isEqualTo("test@example.com");
}

@Test
void 잘못된_이메일로_사용자_생성시_예외() {
    assertThatThrownBy(() -> new User("invalid", "password", "홍길동"))
        .isInstanceOf(IllegalArgumentException.class);
}

// Service 비즈니스 로직 테스트
@Test
void 이미_존재하는_이메일로_가입시_예외() {
