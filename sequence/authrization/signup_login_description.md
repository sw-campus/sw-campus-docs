...
2. 프론트엔드 → 백엔드: 이메일 인증 요청 (POST /api/auth/email/send)
3. 백엔드 → DB: 이메일 중복 확인
4. DB → 백엔드: 조회 결과 반환
5. [조건] 이메일 사용 가능 시:
   - 백엔드 → DB: 인증 토큰 저장 (유효시간: 1시간)
   - 백엔드: 인증 메일 발송 (인증 링크 포함)
   - 백엔드 → 프론트엔드: 발송 완료 응답
...
7. 사용자: 이메일에서 인증 링크 클릭
8. 백엔드: 인증 토큰 검증 및 인증 완료 처리
9. 백엔드 → DB: 이메일 인증 상태 업데이트 (verified: true)

10. 사용자 → 프론트엔드: "인증 확인" 버튼 클릭
11. 프론트엔드 → 백엔드: 인증 상태 확인 (GET /api/auth/email/status)
12. 백엔드 → 프론트엔드: 인증 완료 응답
13. 프론트엔드 → 사용자: 나머지 정보 입력 폼 활성화
14. 사용자 → 프론트엔드: 회원가입 폼 제출 (비밀번호, 기타 정보)
15. 프론트엔드 → 백엔드: 회원가입 API 요청 (POST /api/auth/signup)
16. 백엔드 → DB: 사용자 정보 저장 (비밀번호는 BCrypt 암호화)
17. DB → 백엔드: 저장 완료
18. 백엔드 → 프론트엔드: 회원가입 성공 응답
19. 프론트엔드 → 사용자: 회원가입 성공, 로그인 페이지로 이동
```
...
12. 백엔드 → 프론트엔드: 인증 완료 응답
13. 프론트엔드 → 사용자: 나머지 정보 입력 폼 활성화
14. 사용자 → 프론트엔드: 회원가입 폼 제출 (비밀번호, 사업자등록번호, 사업주명, 사업자등록일)
15. 프론트엔드 → 백엔드: 공급자 회원가입 API 요청 (POST /api/auth/signup/provider)
16. 백엔드 → DB: 공급자 정보 저장 (비밀번호는 BCrypt 암호화)
17. DB → 백엔드: 저장 완료
18. 백엔드 → 프론트엔드: 회원가입 성공 응답
19. 프론트엔드 → 사용자: 회원가입 성공, 로그인 페이지로 이동
```
