...
5. [조건] 스크랩 되어있지 않은 경우 (추가):
   ...
   - [조건] 스크랩 개수 < 10:
     ...
     - DB → 백엔드: 저장 완료
     - 백엔드 → Redis: 캐시 삭제 (wishlist:{userId})
...
3. 백엔드 → Redis: 캐시 조회 (wishlist:{userId})
4. [조건] 캐시 Hit (데이터 있음):
   - Redis → 백엔드: 캐시된 스크랩 목록 반환
   - 백엔드 → 프론트엔드: 200 OK (스크랩 목록 데이터)
5. [조건] 캐시 Miss (데이터 없음):
   - Redis → 백엔드: 캐시 없음
   - 백엔드 → DB: 스크랩 목록 조회 (userId, 스크랩 순서 정렬)
   - DB → 백엔드: 스크랩 목록 반환 (최대 10개)
   - 백엔드 → Redis: 캐시 저장 (wishlist:{userId}, TTL: 10분)
   - 백엔드 → 프론트엔드: 200 OK (스크랩 목록 데이터)
...
- Redis 캐시 키: wishlist:{userId}
- Redis 캐시 TTL: 10분
- 캐시 무효화: 스크랩 추가/삭제 시 해당 유저의 캐시 삭제
...
| 항목 | 설명 |
|------|------|
| 캐시 대상 | 스크랩 목록 조회 (GET /api/wishlists) |
| 캐시 키 | wishlist:{userId} |
| TTL | 10분 |
| 무효화 시점 | 스크랩 추가/삭제 시 |
| 무효화 방식 | 해당 유저의 캐시 삭제 |
| 캐시 Miss 처리 | DB 조회 → Redis 저장 → 응답 |

#### TTL 10분 선정 이유
- 스크랩 데이터는 자주 변경되지 않음
- 변경 시 즉시 캐시 무효화하므로 TTL은 "무효화 실패 시 안전장치" 역할
- 10분은 데이터 신선도와 DB 부하 감소의 균형점
- 운영 후 모니터링하며 조정 가능 (5분~30분 범위 권장)
